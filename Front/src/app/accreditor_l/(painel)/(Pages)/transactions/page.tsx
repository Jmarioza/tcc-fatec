...
export default function TransactionsPage() {
  ...
  async function fetchTransactions(search: SearchParams) {
    try {
      ...
      const beneficiaryGroupSearch =
        search.beneficiaryGroupId || beneficiaryGroups.map((item) => item.id)
        ...
        try {
          const [accreditedsData, productsGroupsData, beneficiariesGroupData] =
            await Promise.all([
              ...
              beneficiaryGroupService.getByUser({
                accreditorId: accreditor.id,
              }),
            ])
    ...
    if (beneficiaryGroupId) {
      ;(async function () {
        try {
          ...
          const userGroups = await userAccreditorBeneficiaryGroupService.get({
            userId,
            accreditorId: accreditor.id,
          })
          ...
          if (data && userGroups) {
            setBeneficiarySubGroups(
              data.filter((item) =>
                userGroups.find(
                  (i) => item.beneficiaryGroupId === i.beneficiaryGroupId,
                ),
              ),
            )
          }
